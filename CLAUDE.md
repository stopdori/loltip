...
## 답변 로그 저장 규칙 (예외 없는 기본 동작)

사용자가 터미널 화면을 긁어서 복붙할 때 글자가 깨지는 문제가 있어서,
화면 출력이 아니라 파일을 통해 내용을 전달받기 위한 규칙이다.
"txt로 저장해줘" 같은 명시적 요청이 없어도 항상 적용된다.

- Claude Code가 채팅에 어떤 답변이든 작성하면, 그 즉시 **같은 내용을**
  `logs/YYYY-MM-DD_HHmm_<작업명 요약>.txt` 파일에 그대로 저장한다
  (Write 도구 등으로). 요약·축약하지 말고, 채팅에 쓴 답변과 완전히
  동일한 텍스트를 저장할 것.
- 예외: 사용자가 순수하게 대화/질문/설명만 요청해서 코드 실행이나
  검증이 전혀 없었던 턴은 저장하지 않아도 됨. 다만 bash 명령을
  하나라도 실행했거나, 파일을 읽거나 수정했거나, 어떤 형태로든
  검증/확인 작업을 한 턴이라면 그 결과를 설명하는 답변은 반드시
  파일로도 저장한다.
- 각 답변 끝에 `→ logs/파일명.txt 저장 완료`라고 한 줄 덧붙여서,
  사용자가 매번 저장 여부를 물어보지 않아도 알 수 있게 한다.
- `logs/`는 `.gitignore`에 등록되어 있음 (로컬 작업 기록용, 커밋 대상 아님).
