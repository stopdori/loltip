...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!pair) return { title: "매치업 | LOLTIP" };

  const [a, b] = pair;
  const champA = CHAMPIONS.find((c) => c.id === a);
  const champB = CHAMPIONS.find((c) => c.id === b);
  if (!champA || !champB) return { title: "매치업 | LOLTIP" };

  const title = `${champA.ko} vs ${champB.ko} 매치업 - LOLTIP`;
  const description = `${champA.ko}와 ${champB.ko}의 스킬 상성, CC 판정, 저지불가 상호작용 정리`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://www.loltip.com/matchup/${slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://www.loltip.com/matchup/${slug}`,
      type: "website",
...
export default async function Page({ params, searchParams }: Props) {
  ...
  // first 파라미터로 나/상대 결정 (없으면 알파벳 앞쪽이 나)
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "챔피언 목록", item: "https://www.loltip.com/champ" },
      {
        ...
        item: `https://www.loltip.com/matchup/${slug}`,
      },
    ],
  };
