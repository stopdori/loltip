/**
 * Fixed translation script for all ahri matchup files
 * Processes mismatches.json and updates en arrays
 *
 * arrayIndex in mismatches.json:
 *   1 = ahri's highlightsByChamp en array (1st highlightsByChamp champ)
 *   2 = opponent's highlightsByChamp en array (2nd highlightsByChamp champ)
 *
 * The en: arrays in highlightsByChamp use 6-space indentation
 * (summary uses 4-space, so won't be counted)
 * Occurrence mapping: arrayIndex 1 -> occurrence 0, arrayIndex 2 -> occurrence 1
 */
...
// Find the nth occurrence (0-indexed) of pattern in str, starting from searchFrom
function findNthOccurrence(str, pattern, n, searchFrom = 0) {
  let count = 0;
  let pos = searchFrom;
  while (true) {
    const found = str.indexOf(pattern, pos);
    if (found === -1) return -1;
...
// Replace the en array at a specific arrayIndex in the file content
// arrayIndex: 1 = ahri's section (0th 6-space en), 2 = opponent's section (1st 6-space en)
function replaceEnArray(content, arrayIndex, enArray) {
  const targetOccurrence = arrayIndex - 1; // convert to 0-indexed

  // Find the targetOccurrence-th '      en:' (6-space indent)
  const enPos = findNthOccurrence(content, '      en:', targetOccurrence);
  if (enPos === -1) {
    return { content, success: false, error: `Could not find en at occurrence ${targetOccurrence}` };
  }

  // Find the [ after en:
  const afterEn = content.indexOf('[', enPos + 5);
  if (afterEn === -1) {
    return { content, success: false, error: 'Could not find [ after en:' };
  }
  ...
  const newContent = content.substring(0, afterEn) + newEnValue + content.substring(closeBracket + 1);
  return { content: newContent, success: true, oldValue: oldEnValue, newValue: newEnValue };
}
...
for (const [relPath, indexMap] of Object.entries(translations)) {
  ...
  // Sort entries by arrayIndex to process in order
  const sortedEntries = Object.entries(indexMap).sort((a, b) => parseInt(a[0]) - parseInt(b[0]));

  for (const [arrayIndexStr, enArray] of sortedEntries) {
    const arrayIndex = parseInt(arrayIndexStr);
    const result = replaceEnArray(content, arrayIndex, enArray);

    if (!result.success) {
      const errMsg = `WARNING: ${relPath} arrayIndex=${arrayIndex}: ${result.error}`;
      console.log(errMsg);
      errors.push(errMsg);
    } else {
      console.log(`Updated ${relPath} arrayIndex=${arrayIndex}: ${result.oldValue} -> ${result.newValue}`);
